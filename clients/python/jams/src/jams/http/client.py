...
class HttpClient:
    ...
    def predict(self, model_name: str, model_input: str) -> type.Prediction:
        ...
        request = models.PredictRequest(model_name=model_name, input=model_input).dict()
        ...
        try:
            ...
            try:
                resp_obj = models.PredictResponse.parse_obj(response.json())
    ...
    def add_model(self, model_name: str) -> None:
        ...
        request = models.AddModelsRequest(model_name=model_name).dict()
        try:
            response = requests.post(url, json=request)
            if response.status_code != 200:
    ...
    def update_model(self, model_name: str) -> None:
        url = f"{self.base_url}/api/models"
        request = models.UpdateModelsRequest(model_name=model_name).dict()
        try:
    ...
    def get_models(self) -> models.GetModelsResponse:
        ...
        try:
            return models.GetModelsResponse.parse_obj(response.json())
        except ValueError as e:
